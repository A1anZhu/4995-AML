...
plt.show()
print("I will drop the RowNumber, CustomerId, and surname columns because they have no relationship with exiting or not. ")
...
print("We do not fneed to apply scaling because the decision tree will split the data in to different sections with a certain line. Even after scaling, the separted porportion is still the same, so there is no need to scale.")
...
print(f"For the development set the recall score is {recall_score(bank_y_val,bank_y_val_pred,pos_label=1)}, the precision score is {precision_score(bank_y_val,bank_y_val_pred,pos_label=1)}, the F1-Score is {f1_score(bank_y_val,bank_y_val_pred,pos_label=1)}.")
print(f"For the test set the recall score is {recall_score(bank_y_test,bank_y_test_pred,pos_label=1)}, the precision score is {precision_score(bank_y_test,bank_y_test_pred,pos_label=1)}, the F1-Score is {f1_score(bank_y_test,bank_y_test_pred,pos_label=1)}.")
...
print(pure)
print("All trees in the trained random forest model have pure leaves. I iterate through every leave of every tree in the forst and check their impurity. If the impurity is not equal to 0, the leaf is not pure, since a non-zero impurity implies the leave has more than one category.")
...
## YOUR ANSWER HERE
print(f"For n_estimators, I will choose 200, 300, 400, 500, and 600. For max_depth, I choose 10, 15, 20, 25, and 30. As the value of n_estimators increases and the other remains the same, the performance should be better. The default n_estimator is 100. I would like to increase the number.The default of max_depth is None, which sets no limits on the tree. The depth of the decision tree is {dtc.tree_.max_depth} in 1.6. I believe the range of depth should be around or lower than this number.")
...
print(f"The score of best performace is {np.max(scores)} with n_estimators= {best_n} and max_depth = {best_depth}. The performance is better than in Q2.1")
...
ax.tick_params(axis='x',rotation=90)
plt.show()
print("Age, NumofProducts, and Balance are the top 3 most important features. The difference compare to Q2.5 is that isActivement is replaced by Balance. I think Balance makes more sense because the performance of model in Q2.5 is higher than in Q1.9.")
...
print(f"The performance of xgboosting is {xgb_best_score} spending {xgb_time.total_seconds()} seconds. The xgboosting has higher score compare to 3.1 but spending more time.")
...
## YOUR ANSWER HERE
print(f"The XGBoost performs the best with a score of {xgb_best_score}, and Decision Tree in 1.6 with pure leaves perform the worst with a score of {precision_score(bank_y_test,bank_y_test_pred)}. I would choose HistGradientBoostingClassifier because it performs almost as good as XGBoost and uses much less time.")
...
xgb_best_score = np.max(xgb_scores) 
print(f"The performance of xgboosting is {xgb_best_score} which is almost the same as {xgb_best_score} in Q3.2. The model performs as good as in Q3.2. This is because the model is most influence by the top features. The variance of less important features do not have large impact of the model and its performance. Thus, the performance is almost the same.")
